...
tries to guess that letter again. Remember to stop the game when all the
letters have been guessed correctly! Don’t worry about choosing a word randomly
or keeping track of the number of guesses the player has remaining - we will
...
guess_word = ['_' for _ in range(0, len(random_word))]
while True:
    print(' '.join(guess_word))
    if '_' not in guess_word:
        break
    guess_char = input('Provide letter: ')
    char_indexes = [idx for idx, elem in enumerate(random_word) if elem == guess_char]
    if char_indexes:
        for char_idx in char_indexes:
            guess_word[char_idx] = guess_char
    else:
        print('Incorrect!')
